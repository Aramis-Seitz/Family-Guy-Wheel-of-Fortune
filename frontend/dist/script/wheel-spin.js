...
function getSegmentIndex(rotation, stepAngle) {
...
function performSpinStep(step, config) {
    ...
    if (delay > DRUMROLL_DELAY_THRESHOLD)
        playDrumRoll();
    setTimeout(() => performSpinStep(step, config), delay);
}
export function spinWheel(totalSteps, direction) {
    spinCancelled = false;
...
function logSpinDetails(ranNum, multiplier, boostedValue) {
    console.log("Number from fs server:", ranNum);
    console.log("Multiplier:", multiplier);
    console.log("Boosted value:", boostedValue);
}
export async function spinWheelWithRandomSteps(direction) {
    try {
...
export function initMultiplierSlider() {
