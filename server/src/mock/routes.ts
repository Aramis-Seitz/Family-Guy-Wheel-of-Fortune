...
mockRouter.post('/auth/signup', (req, res) => {
  ...
  if (!email || !password || !username) {
    res.status(400).json({ error: 'email, password and username are required', code: ERROR_CODES.VALIDATION });
    return;
  }

  if (findProfileByEmail(email)) {
    res.status(409).json({ error: 'This email is already registered', code: ERROR_CODES.CONFLICT });
    return;
  }
...
mockRouter.post('/auth/login', (req, res) => {
  ...
  if (!profile || profile.password !== password) {
    res.status(401).json({ error: 'Invalid email or password', code: ERROR_CODES.UNAUTHORIZED });
    return;
  }
...
mockRouter.get('/profile/by-username/:username', (req, res) => {
  ...
  if (!profile) { res.status(404).json({ error: 'Not found', code: ERROR_CODES.NOT_FOUND }); return; }
...
mockRouter.post('/profile', (req, res) => {
  ...
  if (findProfile(id)) {
    ...
    res.status(409).json({ error: 'Already exists', code: ERROR_CODES.CONFLICT });
    return;
  }
