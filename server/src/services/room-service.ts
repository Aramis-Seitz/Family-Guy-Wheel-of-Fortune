...
export async function createRoom(userId: string): Promise<CreateRoomResponseBody> {
    ...
    return {
        roomKey,
        players: [formatDisplayName(hostUsername, hostSuffix)],
        names: []
    };
}
...
function toDisplayNames(players: RoomPlayer[]): string[] {
    return players.map((player) =>
        formatDisplayName(player.username, player.suffix)
    );
}
...
function requireRoomHost(room: RoomData | null, userId: string, action: string): asserts room is RoomData {
    ...
    if (room.host_id !== userId) {
        throw new AppError(`Only the host may ${action}`, 403);
    }
}
...
export async function joinRoom(
    ...
    const updatedPlayers = currentPlayers.some(
    ...
    const hostPlayer =
        room.players.find((player) => player.id === room.host_id) ??
        players[0];

    const hostName = hostPlayer
        ? formatDisplayName(hostPlayer.username, hostPlayer.suffix)
        : "";
    ...
    return {
        players: toDisplayNames(players),
        multiplier,
        names,
        hostName
    };
...
export async function spinRoom(
    userId: string,
    roomKey: string,
    direction: string
...
export async function setMultiplier(
    userId: string,
    roomKey: string,
    multiplier: number
): Promise<void> {
    ...
    if (!Number.isFinite(multiplier)) {
        throw new AppError("Invalid multiplier", 400);
    }
